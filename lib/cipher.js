...
function encrypt(data) {
    ...
    let cipher = crypto.createCipheriv(cipherAlgorithm, process.env.APP_KEY, iv);
...
function decrypt(data) {
    ...
    let cipher = crypto.createDecipheriv(cipherAlgorithm, process.env.APP_KEY, iv);
...
function hmac(data, iv) {
    return crypto.createHmac(hmacAlgorithm, process.env.APP_KEY).update(iv + data).digest(encoding);
}
